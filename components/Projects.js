  ...
  return (
    <section id="projects" className="py-20 space-y-12">
      <div className="flex justify-between items-end">
        ...
        <a href="#" className="text-primary hover:underline font-semibold flex items-center gap-2 group">
          View Portfolio 
        ...
        {projects.map((project, idx) => (
          ...
          >
            <div className="h-56 overflow-hidden relative z-0">
              ...
              />
              <div className="absolute inset-0 bg-slate-900/70 opacity-0 group-hover:opacity-100 transition-all duration-500 flex items-center justify-center backdrop-blur-[4px] z-10 pointer-events-none group-hover:pointer-events-auto">
            ...
            <div className="p-6 space-y-4">
              <h4 className="text-foreground font-bold text-xl">{project.title}</h4>
              <p className="text-on-surface-variant text-sm line-clamp-2 leading-relaxed">
                {project.description}
              </p>
              <div className="flex gap-2">
                {project.tags.map(tag => (
                  <span key={tag} className="text-[10px] px-2 py-0.5 rounded bg-primary-container/20 text-primary font-bold tracking-tighter">
                    {tag}
                  </span>
                ))}
              </div>
